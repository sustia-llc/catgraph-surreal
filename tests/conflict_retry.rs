...
//! the other:
//!
//! - The **in-memory** engine aborts on *read* conflicts as well as write
//!   conflicts, so it aborts more often than plain snapshot isolation would.
...
//!   at the commit call.
//!
//! The second is the one that shapes the code: a retry wrapped around a single
...
mod memory {
    ...
    async fn conflicting_increments_all_land() {
        super::conflicting_increments_all_land(store("conflict_all_land").await).await;
    }
